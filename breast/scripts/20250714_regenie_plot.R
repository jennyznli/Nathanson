...
dat <- fread(here("breast", "data", "EUR_step2_chr1_Y1.regenie"), header = TRUE)
...
# Get indices of non-significant SNPs
nonsig_indices <- which(dat$PVAL > p.thresh)
x <- length(nonsig_indices)

# Sample indices, not IDs
s_indices <- sample(nonsig_indices, round(k.thresh * x), replace=F)

print("creating manhattan plot...")
png(MANOUTFILE, height=600, width=1200)
# Combine sampled non-significant indices with all significant indices
sig_indices <- which(dat$PVAL <= p.thresh)
keep_indices <- c(s_indices, sig_indices)
y <- dat[keep_indices, ]
