...
# Generate REGENIE analysis report (one per phenotype in PHENOTYPES)
rule generate_report:
    input:
        single_variant=expand(f"{RUN_DIR}/output/{PROJECT}.chr{{CHR}}.step2_single_variant_{{{{PHENO}}}}.regenie", CHR=CHROMOSOMES),
        gene_based=expand(f"{RUN_DIR}/output/{PROJECT}.chr{{CHR}}.step2_gene_based_{{{{PHENO}}}}.regenie", CHR=CHROMOSOMES),
        pheno=f"{RUN_DIR}/input/{PROJECT}.regenie.pheno.txt",
        rmd="report_regenie.Rmd"
    output:
        html=f"reports/{PROJECT}_{{PHENO}}_regenie_report.html",
        csv_single=f"reports/{PROJECT}_{{PHENO}}_single_variant_results.csv",
        csv_gene=f"reports/{PROJECT}_{{PHENO}}_gene_based_results.csv"
    wildcard_constraints:
    ...
    params:
        ...
        output_dir="reports",
        single_variant_pattern=lambda wildcards: f"{PROJECT}.chr{{CHR}}.step2_single_variant_{wildcards.PHENO}.regenie",
        gene_based_pattern=lambda wildcards: f"{PROJECT}.chr{{CHR}}.step2_gene_based_{wildcards.PHENO}.regenie",
        pheno_file=f"{RUN_DIR}/input/{PROJECT}.regenie.pheno.txt",
        pheno_col=lambda wildcards: wildcards.PHENO,
        report_title=lambda wildcards: f"SIMPLEXO ExWAS Report - {PROJECT} - {wildcards.PHENO}",
    ...
    threads: 1
    log:
        f"{RUN_DIR}/logs/{PROJECT}.{{PHENO}}.generate_report.log"
    shell:
        """
