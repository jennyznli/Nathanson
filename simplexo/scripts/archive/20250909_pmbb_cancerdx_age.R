...
SS_DIR = here("simplexo", "ss")
DATA_DIR = here("simplexo", "data")
flags <- fread(file.path(SS_DIR, "rgcname_pmbbid_metadata_flags.csv"))
progeny_case <- read_excel(file.path(SS_DIR, "br_pts_for_exwas_08292025.xlsx"))
pmcr <- read.csv(file.path(SS_DIR, "pmbb_147_pmcrbreastage.csv"))
...
icds <- unique(read.table(file.path(DATA_DIR, "breast_simplexo3_v2_cancer_filtered_patients_ids.txt"))$V1)
...
write.table(progeny_pmbb_ids, file.path(DATA_DIR, "progeny_ids.txt"), row.names = FALSE, col.names = FALSE, quote = FALSE)
...
icd_cases <- fread(file.path("breast_simplexo3_v2_cancer_filtered_patients.txt"))
icd_case_ids <- readLines(file.path("breast_simplexo3_v2_cancer_filtered_patients_ids.txt"))

all_ids <- sort(unique(c(progeny_pmbb_ids, icd_case_ids)))
write.table(all_ids, file.path(DATA_DIR, "simplexo_pmbb_ids.txt"), row.names = FALSE, col.names = FALSE, quote = FALSE)
...
write.table(merged_data, file.path(DATA_DIR, "simplexo3_case_ages_merged.txt"))

merged_sel <- merged %>%
    select(PMBB_ID, Age) %>%
    filter(!is.na(Age))
dim(merged_sel)
write.table(merged_sel, file.path(DATA_DIR, "simplexo3_case_ages.txt"), quote = FALSE, row.names = FALSE, col.names = FALSE, sep = "\t")
