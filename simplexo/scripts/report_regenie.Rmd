...
if(!params$pheno_col %in% names(pheno)) {
  stop("Phenotype column '", params$pheno_col, "' not found in ", params$pheno_file,
       " (available columns: ", paste(names(pheno), collapse = ", "), ")")
}
...
### Suggestive Gene-Based Associations (Raw p < 0.001, FDR ≥ `r params$fdr_threshold`)
...
# Get top FDR significant genes for labeling (top 10 per mask)
...
**Note:** CHEK2 **missense** sites are excluded from all masks upstream (`mask_gene_consequence_exclude.txt`, applied in `preprocess_regenie.py` via `--gene-consequence-exclude`), so they will not appear below. Excluded sites are written to `<output_prefix>.gene_consequence_excluded.csv`.
...
if(nrow(chek2_results) > 0) {
  ...
  # Save CHEK2 results to CSV FIRST
...
single_file <- file.path(params$output_dir, paste0(params$project, "_all_single_variant_results.csv"))
gene_file <- file.path(params$output_dir, paste0(params$project, "_all_gene_based_results.csv"))
