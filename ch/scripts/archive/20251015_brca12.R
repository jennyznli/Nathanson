...
# STATS
# ========================================================================
case_strata1 %>% filter(Cancer == "0") %>% select(Mutation) %>% table()
case_strata1 %>% filter(Cancer == "1") %>% select(Mutation) %>% table()
case_strata1 %>% select(Mutation) %>% table()
case_strata1 %>% select(Cancer) %>% table()
dim(case_strata1)

case_strata2 %>% select(Mutation) %>% table()
case_strata2 %>% select(Cancer) %>% table()
dim(case_strata2)

control_strata1 %>% select(Cancer) %>% table()
dim(control_strata1)

control_strata2 %>% select(Cancer) %>% table()
dim(control_strata2)
...
#### VALIDATION #####
cat("Total non-BRCA:", length(non_brca12), "\n")
cat("In control strata 1:", nrow(control_strata1), "\n")
cat("In control strata 2:", nrow(control_strata2), "\n")
cat("Missing:", length(non_brca12) - nrow(control_strata1) - nrow(control_strata2), "\n")

# Check for NAs in key columns
cat("\nCase Strata 1 - Missing Sample_Age:", sum(is.na(case_strata1$Sample_Age)), "\n")
cat("Case Strata 1 - Missing Dx_Age (cancer only):",
    sum(is.na(case_strata1$Dx_Age[case_strata1$Cancer == 1])), "\n")

both_mutations <- brca12_df %>%
    filter(PMBB_ID %in% all_brca1_crep_ids & PMBB_ID %in% all_brca2_crep_ids)
cat("\nPeople with both BRCA1 and BRCA2:", nrow(both_mutations), "\n")
# 1 person has both


#### DAEHYUN SAMPLES #####
aml_ids <- sort(unique(aml$person_id))
aml_ch <- var %>% filter(Sample.ID %in% aml_ids)
dim(aml_ch)
var$Sample.ID
aml_cov <- cov %>% filter(person_id %in% aml_ids)
dim(aml_cov)
# don't have the variant annotation data bc they're not in CREP

# all in freeze 2.0
aml_ids %in% old_brca2
aml_ids %in% old_brca1
# none are BRCA1/2 carriers
