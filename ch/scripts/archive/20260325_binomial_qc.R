...
# ========================
# BINOMIAL GERMLINE - TET2
# ========================


# test if sig dif from 0.5
tet2 <- tet2 %>%
    mutate(
        ...
        is_germline = !is.na(binom_p) & binom_p >= 0.01  # high p = consistent with 0.5 = germline
    )
#
# cat("Variants failing binomial test:", sum(tet2$binom_fail, na.rm = TRUE), "\n")
# cat("Variants passing:", sum(!tet2$binom_fail, na.rm = TRUE), "\n")
# i think i can justmanulll do i t

# CBL: RING finger missense p.381-421
all_ch %>% filter(Gene == "CBL", wl.exception == TRUE) %>% select(Sample.AltFrac)
dim(cbl)
# 1...
# super low VAF.. we're fine
