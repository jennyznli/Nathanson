...
# add age squared
base_covs_squared <- "Sample_age + Sample_age2 + Sequenced_gender + Batch +
                      Smoke_History +
                      PC1 + PC2 + PC3 + PC4 + PC5 + PC6"
...
#
# Model 1: CHIP_Binary ~ BRCA12_Case + Sample_age + Sequenced_gender + Batch +
...
# 1      2593     1318.2
# 2      2590     1315.3  3   2.8573 0.9472 0.4169
...
cat("\n=== LRT: spline vs squared (BRCA1/2) ===\n")
print(anova(fit_m1s_12, fit_m1sq_12, test = "F"))
# 0.2478

cat("\n=== LRT: linear vs squared (BRCA1/2) ===\n")
print(anova(fit_m1_12, fit_m1sq_12, test = "F"))
# 0.8241

# so linear is fine.. .none of them make a good difference
...
firth_12 <- logistf(as.formula(paste("CHIP_Binary ~ BRCA12_Case +", base_covs_linear)), data = cov)
firth_1  <- logistf(as.formula(paste("CHIP_Binary ~ BRCA1_Case +",  base_covs_linear)), data = cov)
firth_2  <- logistf(as.formula(paste("CHIP_Binary ~ BRCA2_Case +",  base_covs_linear)), data = cov)
...
print(anova(fit_m1_12, fit_int_age, test = "F"))
# Analysis of Deviance Table
#
# Model 1: CHIP_Binary ~ BRCA12_Case + Sample_age + Sequenced_gender + Batch +
#     Smoke_History + PC1 + PC2 + PC3 + PC4 + PC5 + PC6
# Model 2: CHIP_Binary ~ BRCA12_Case * Sample_age + Sequenced_gender + Batch +
#     Smoke_History + PC1 + PC2 + PC3 + PC4 + PC5 + PC6
# Resid. Df Resid. Dev Df Deviance      F   Pr(>F)
# 1      2593     1318.2
# 2      2592     1311.3  1   6.8843 6.8378 0.008977 **
#     ---
#     Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1

cat("\n=== Interaction: Carrier × Sex ===\n")
print(anova(fit_m1_12, fit_int_sex, test = "F"))
# Analysis of Deviance Table
#
# Model 1: CHIP_Binary ~ BRCA12_Case + Sample_age + Sequenced_gender + Batch +
#     Smoke_History + PC1 + PC2 + PC3 + PC4 + PC5 + PC6
# Model 2: CHIP_Binary ~ BRCA12_Case * Sequenced_gender + Sample_age + Sequenced_gender +
#     Batch + Smoke_History + PC1 + PC2 + PC3 + PC4 + PC5 + PC6
# Resid. Df Resid. Dev Df Deviance      F  Pr(>F)
# 1      2593     1318.2
# 2      2592     1313.8  1   4.3467 4.3708 0.03666 *
#     ---
#     Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
#
# ============================================================
...
fit_m5 <- multinom(
    as.formula(paste("CHIP_Binary ~ Carrier3 +", base_covs_linear)),
    data = cov, weights = weights, trace = FALSE
)
summary(fit_m5)
# Call:
#     multinom(formula = as.formula(paste("CHIP_Binary ~ Carrier3 +",
#                                         base_covs_linear)), data = cov, weights = weights, trace = FALSE)
#
# Coefficients:
#     Values   Std. Err.
# (Intercept)           -5.70343524  0.58777977
# Carrier3BRCA1          0.55647862  0.21875874
# Carrier3BRCA2          0.39126225  0.22755506
# Sample_age             0.03183107  0.00579917
# Sequenced_genderMale  -0.42377165  0.21248364
# Batch                  0.84638123  0.24636223
# Smoke_History         -0.11282993  0.17335306
# PC1                   -4.66980203  5.71098584
# PC2                    6.20439782  4.68327770
# PC3                   -0.01363057  7.28757561
# PC4                    1.38789157  7.78015919
# PC5                    2.11535240  9.02237864
# PC6                  -18.12939864 10.71855941
#
# Residual Deviance: 1317.728
# AIC: 1343.728
