        ...
        if no_sample:
            ...
            for i,sample_name in enumerate(vcf_reader.samples):
                ...
                sample_data.append({
                    ...
                    'GQ':record.format('GQ')[i] if record.format('GQ') is not None else '.'
                })
...
def report_header(tumor_normal=False,no_caller=False):
    ...
    header+=['Gene','Gene.Accession','Variant.LoF_level','Variant.Category','Variant.Class','Variant.Consequence',
            ...
            'CANONICAL','MANE_SELECT','MANE_PLUS_CLINICAL',  # newly added 
    ...
    if tumor_normal:
        header+=['Tumor.Zyg','Tumor.Depth','Tumor.AltDepth','Tumor.AltFrac', 'Tumor.GQ',
                'Normal.Zyg','Normal.Depth','Normal.AltDepth','Normal.AltFrac', 'Normal.GQ']
    else:
        header+=['Sample.Zyg','Sample.Depth','Sample.AltDepth','Sample.AltFrac', 'Sample.GQ']
...
def main(argv=None):
    ...
    if args.mode=='no_sample':
        for x in ['Sample.ID','Sample.Zyg','Sample.Depth','Sample.AltDepth','Sample.AltFrac', 'Sample.GQ']:
            header.remove(x)
                    ...
                    if csq_dict.get('IMPACT','MODIFIER')=='HIGH':
                        ...
                        if process_annotation(vep_data,csq_dict,tumor_normal,single,tumor,normal,sample):
                            ...
                            if filter_result==True:
                                ...
                                skipped_filters+=1
            if not high_impact:
                # Process first canonical OR MANE transcript for each gene
                for csq_i in record.INFO['ANN'].split(','):
                    csq_dict=dict(zip(ann_header,csq_i.split('|')))
                    is_canonical = csq_dict.get('CANONICAL','') == 'YES'
                    has_mane_select = csq_dict.get('MANE_SELECT','') != ''
                    has_mane_plus = csq_dict.get('MANE_PLUS_CLINICAL','') != ''
                    has_symbol = csq_dict.get('SYMBOL','') != ''
                    
                    # Keep if has symbol AND (canonical OR has MANE annotation)
                    if has_symbol and (is_canonical or has_mane_select or has_mane_plus):
                        if process_annotation(vep_data,csq_dict,tumor_normal,single,tumor,normal,sample):
                            vep_data.fill_values(header)
                            filter_result=should_report_variant(vep_data,gene_filter,gene_list,region_filter,bed_regions,record,blacklist_filter,blacklist)
                            if filter_result==True:
                                ...
                                skipped_filters+=1
                        break  # Still only take first match
