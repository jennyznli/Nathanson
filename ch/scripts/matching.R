...
# ========================
# PROPENSITY SCORE MATCHING - 1:4 RATIO
# ========================
m.out4 <- matchit(
    match_formula,
    data = unaff,
...
pre <- as.data.frame(m.out4.summary$sum.all)
post <- as.data.frame(m.out4.summary$sum.matched)

pre$Covariate <- rownames(pre)
post$Covariate <- rownames(post)

balance_table <- pre %>%
    dplyr::select(Covariate,
           Mean_Treated_Pre   = `Means Treated`,
           Mean_Control_Pre   = `Means Control`,
           SMD_Pre            = `Std. Mean Diff.`,
           VR_Pre             = `Var. Ratio`) %>%
    left_join(
        post %>%
            dplyr::select(Covariate,
                   Mean_Treated_Post  = `Means Treated`,
                   Mean_Control_Post  = `Means Control`,
                   SMD_Post           = `Std. Mean Diff.`,
                   VR_Post            = `Var. Ratio`),
        by = "Covariate"
    )
...
balance_table <- balance_table %>%
    dplyr::mutate(Covariate = case_when(
        Covariate == "distance"              ~ "Propensity score",
        Covariate == "Sample_age"            ~ "Age (years)",
        Covariate == "Sequenced_genderFemale"~ "Sex: Female",
        Covariate == "Sequenced_genderMale"  ~ "Sex: Male",
        Covariate == "Smoke_History0"        ~ "Non-smoker",
        Covariate == "Smoke_History1"        ~ "Ever smoker",
        Covariate == "Batch1"               ~ "Batch 1",
        Covariate == "Batch2"               ~ "Batch 2",
        grepl("^PC", Covariate)             ~ Covariate,
        TRUE                                ~ Covariate
    )) %>%
    # Add covariate type
    mutate(Type = case_when(
        Covariate == "Propensity score"        ~ "Distance",
        Covariate == "Age (years)"             ~ "Continuous",
        grepl("^PC", Covariate)               ~ "Continuous",
        TRUE                                   ~ "Binary"
    )) %>%
    # Round numeric columns
    mutate(across(where(is.numeric), ~ round(., 3))) %>%
    # Replace Inf/NaN variance ratios (binary vars) with NA
    mutate(across(contains("VR"), ~ ifelse(is.infinite(.) | is.nan(.), NA, .))) %>%
    # Reorder columns
    dplyr::select(Covariate, Type,
           Mean_Treated_Pre, Mean_Control_Pre, SMD_Pre, VR_Pre,
           Mean_Treated_Post, Mean_Control_Post, SMD_Post, VR_Post)

sample_sizes <- data.frame(
    Covariate         = c("N (Carriers)", "N (Controls)"),
    Type              = NA,
    Mean_Treated_Pre  = c(697, 28125),
    Mean_Control_Pre  = NA,
    SMD_Pre           = NA,
    VR_Pre            = NA,
    Mean_Treated_Post = c(677, 2327),
    Mean_Control_Post = NA,
    SMD_Post          = NA,
    VR_Post           = NA
)

balance_table_final <- bind_rows(balance_table, sample_sizes)

write_xlsx(balance_table_final,
          file.path("ch", "data", "supp_matching_balance_table.xlsx"))
