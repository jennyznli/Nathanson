...
# Create the file_info object with the paths you want to process
file_info <- tibble(
    path = list.files(base_dir, pattern = "\\.csv$", full.names = TRUE)
) %>%
    mutate(
        # Extract the ID (PMBB ID before first period)
        id = str_extract(basename(path), "^[^.]+"),

        # Extract the caller (the part between first and second period)
        caller = str_extract(basename(path), "(?<=\\.).*?(?=\\.)")
    )
...
ggsave(file.path("ch", "figures", "variant_counts_individual.pdf"), p1_individual, width = 10, height = 6)
ggsave(file.path("ch", "figures", "variant_counts_combined.pdf"), p1_combined, width = 8, height = 6)
...
ggsave(file.path("ch", "figures", "venn_diagrams_individual.pdf"), p2_individual, width = 15, height = 15)
ggsave(file.path("ch", "figures", "venn_diagram_combined.pdf"), p2_combined, width = 8, height = 6)
...
ggsave(file.path("ch", "figures", "top_genes_combined.pdf"), p4, width = 8, height = 10)
...
p5_combined <- ggplot(af_comparison_combined, aes(x = mutect2, y = vardict)) +
    ...
    theme_bw()
ggsave(file.path("ch", "figures", "af_scatter_mutect2_vardict_combined.pdf"), p5_combined, width = 6, height = 4)
...
p5_combined <- ggplot(af_comparison_combined, aes(x = mutect2, y = pmbb)) +
    ...
    theme_bw()
ggsave(file.path("ch", "figures", "af_scatter_mutect2_pmbb_combined.pdf"), p5_combined, width = 6, height = 4)
...
p5_combined <- ggplot(af_comparison_combined, aes(x = vardict, y = pmbb)) +
    ...
    theme_bw()
ggsave(file.path("ch", "figures", "af_scatter_vardict_pmbb_combined.pdf"), p5_combined, width = 6, height = 4)
...
#           file.path("ch", "output", "discordant_mutect2_vardict.csv"),
#           row.names = FALSE)
# write.csv(discordant_mutect2_pmbb,
#           file.path("ch", "output", "discordant_mutect2_pmbb.csv"),
#           row.names = FALSE)
# write.csv(discordant_vardict_pmbb,
#           file.path("ch", "output", "discordant_vardict_pmbb.csv"),
          # row.names = FALSE)
...
p6_individual <- ggplot(af_data, aes(x = AF, fill = caller)) +
    ...
    theme_bw()
ggsave(file.path("ch", "figures", "af_density_individual.pdf"), p6_individual, width = 8, height = 6)
...
p6_combined <- ggplot(af_data, aes(x = AF, fill = caller)) +
    ...
    theme_bw() +
    theme(legend.position = "bottom")
ggsave(file.path("ch", "figures", "af_density_combined.pdf"), p6_combined, width = 8, height = 6)
...
p7_combined <- ggplot(depth_data, aes(x = TotalDepth, fill = caller)) +
    ...
    labs(title = "Read Depth Distribution (All Samples)",
         x = "log10(Total Depth)", y = "Count") +
    theme_bw()
ggsave(file.path("ch", "figures", "depth_histogram_individual.pdf"), p7_individual, width = 13, height = 10)
ggsave(file.path("ch", "figures", "depth_histogram_combined.pdf"), p7_combined, width = 10, height = 6)
...
ggsave(file.path("ch", "figures", "depth_boxplot_combined.pdf"), p7_box, width = 8, height = 6)
...
ggsave(file.path("ch", "figures", "total_depth_histogram_individual.pdf"), p7_individual, width = 13, height = 10)
ggsave(file.path("ch", "figures", "total_depth_histogram_combined.pdf"), p7_combined, width = 10, height = 6)
ggsave(file.path("ch", "figures", "total_depth_boxplot_combined.pdf"), p7_box, width = 8, height = 6)
...
ggsave(file.path("ch", "figures", "alt_depth_histogram_individual.pdf"), p8_individual, width = 13, height = 10)
ggsave(file.path("ch", "figures", "alt_depth_histogram_combined.pdf"), p8_combined, width = 10, height = 6)
ggsave(file.path("ch", "figures", "alt_depth_boxplot_combined.pdf"), p8_box, width = 8, height = 6)
...
write_csv(ch_all2, file.path("ch", "data", "pilot_somatic_ch.csv"))
...
write_csv(ch_all2_mutect2, file.path("ch", "data", "pilot_mutect2_ch.csv"))
write_csv(ch_all2_vardict, file.path("ch", "data", "pilot_vardict_ch.csv"))
write_csv(ch_all2_pmbb, file.path("ch", "data", "pilot_pmbb_ch.csv"))
...
write_csv(mutect2_variants, file.path("ch", "data", "pilot_mutect2_all.csv"))
write_csv(vardict_variants, file.path("ch", "data", "pilot_vardict_all.csv"))
write_csv(pmbb_variants, file.path("ch", "data", "pilot_pmbb_all.csv"))
write_csv(x, file.path("ch", "data", "pilot_somatic_all.csv"))
