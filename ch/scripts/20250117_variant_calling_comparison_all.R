...
# ========================
# LOAD AND FORMAT VEP DATA FROM SINGLE FILE
# ========================

# Read the large combined file
vars <- read.csv(file.path("ch", "data", "pilot_somatic_all.csv"))

# Filter for LoF level 1
lof <- vars %>% filter(Variant.LoF_level == 1)
lof$caller <- as.factor(lof$caller)

# Function to read and convert numeric columns (keep for consistency)
read_and_convert <- function(df) {
...
# Convert data types
lof <- read_and_convert(lof)
lof$PMBB_ID <- lof$vep_sample_id

# Convert to the nested list structure: vep[[sample_id]][[caller]]
vep <- lof %>%
    group_by(PMBB_ID, caller) %>%
    group_split() %>%
    {
        # Create nested list structure
        result <- list()
        for(df in .) {
            pmbb_id <- unique(df$PMBB_ID)
            caller_name <- unique(df$caller)

            if(is.null(result[[pmbb_id]])) {
                result[[pmbb_id]] <- list()
            }
            result[[pmbb_id]][[caller_name]] <- df
        }
        result
    }
...
ggsave(file.path("ch", "figures", "variant_counts_individual_patho.pdf"), p1_individual, width = 10, height = 6)
ggsave(file.path("ch", "figures", "variant_counts_combined_patho.pdf"), p1_combined, width = 8, height = 6)
...
ggsave(file.path("ch", "figures", "venn_diagrams_individual_patho.pdf"), p2_individual, width = 15, height = 15)
ggsave(file.path("ch", "figures", "venn_diagram_combined_patho.pdf"), p2_combined, width = 8, height = 6)
...
pdf(file.path("ch", "figures", "upset_combined_combined_patho.pdf"), width = 10, height = 6)
...
ggsave(file.path("ch", "figures", "top_genes_combined_patho.pdf"), p4, width = 8, height = 10)
...
p5_combined <- ggplot(af_comparison_combined, aes(x = mutect2, y = vardict)) +
    ...
    theme_bw()
ggsave(file.path("ch", "figures", "af_scatter_mutect2_vardict_combined_patho.pdf"), p5_combined, width = 6, height = 4)
...
p5_combined <- ggplot(af_comparison_combined, aes(x = mutect2, y = pmbb)) +
    ...
    theme_bw()
ggsave(file.path("ch", "figures", "af_scatter_mutect2_pmbb_combined_patho.pdf"), p5_combined, width = 6, height = 4)
...
p5_combined <- ggplot(af_comparison_combined, aes(x = vardict, y = pmbb)) +
    ...
    theme_bw()
ggsave(file.path("ch", "figures", "af_scatter_vardict_pmbb_combined_patho.pdf"), p5_combined, width = 6, height = 4)
...
write.csv(discordant_mutect2_vardict,
          file.path("ch", "output", "discordant_mutect2_vardict_patho.csv"),
          row.names = FALSE)
write.csv(discordant_mutect2_pmbb,
          file.path("ch", "output", "discordant_mutect2_pmbb_patho.csv"),
          row.names = FALSE)
write.csv(discordant_vardict_pmbb,
          file.path("ch", "output", "discordant_vardict_pmbb_patho.csv"),
          row.names = FALSE)
...
p6_individual <- ggplot(af_data, aes(x = AF, fill = caller)) +
    ...
    theme_bw()
ggsave(file.path("ch", "figures", "af_density_individual_patho.pdf"), p6_individual, width = 8, height = 6)
...
p6_combined <- ggplot(af_data, aes(x = AF, fill = caller)) +
    ...
    theme_bw() +
    theme(legend.position = "bottom")
ggsave(file.path("ch", "figures", "af_density_combined_patho.pdf"), p6_combined, width = 8, height = 6)
...
p7_combined <- ggplot(depth_data, aes(x = TotalDepth, fill = caller)) +
    ...
    labs(title = "Read Depth Distribution (All Samples)",
         x = "log10(Total Depth)", y = "Count") +
    theme_bw()
ggsave(file.path("ch", "figures", "depth_histogram_individual_patho.pdf"), p7_individual, width = 13, height = 10)
ggsave(file.path("ch", "figures", "depth_histogram_combined_patho.pdf"), p7_combined, width = 10, height = 6)
...
ggsave(file.path("ch", "figures", "depth_boxplot_combined_patho.pdf"), p7_box, width = 8, height = 6)
...
ggsave(file.path("ch", "figures", "total_depth_histogram_individual_patho.pdf"), p7_individual, width = 13, height = 10)
ggsave(file.path("ch", "figures", "total_depth_histogram_combined_patho.pdf"), p7_combined, width = 10, height = 6)
ggsave(file.path("ch", "figures", "total_depth_boxplot_combined_patho.pdf"), p7_box, width = 8, height = 6)
...
ggsave(file.path("ch", "figures", "alt_depth_histogram_individual_patho.pdf"), p8_individual, width = 13, height = 10)
ggsave(file.path("ch", "figures", "alt_depth_histogram_combined_patho.pdf"), p8_combined, width = 10, height = 6)
ggsave(file.path("ch", "figures", "alt_depth_boxplot_combined_patho.pdf"), p8_box, width = 8, height = 6)
...
write_csv(aml_all2, file.path("ch", "data", "pilot_somatic_aml_lof1.csv"))
...
write_csv(aml_all2_mutect2, file.path("ch", "data", "pilot_mutect2_aml_lof1.csv"))
write_csv(aml_all2_vardict, file.path("ch", "data", "pilot_vardict_aml_lof1.csv"))
write_csv(aml_all2_pmbb, file.path("ch", "data", "pilot_pmbb_aml_lof1.csv"))
