...
flags4 <- read.csv(here("PMBB", "4.0", "rgcname_pmbbid_metadata_flags_freeze4.csv"))
flags3 <- read.csv(here("PMBB", "3.0", "rgcname_pmbbid_metadata_flags.csv"))
flags4$PMBB_consent <- as.factor(flags4$PMBB_consent)
consent <- flags4 %>% filter(PMBB_consent %in% c("current", "null"))
...
mrn <- read_excel(here("tgct", "ss", "KT_IDS_MRNs_UIDs.xlsx"))
dim(brad)
...
tgct_res <- tgct_res %>% filter(person_id %in% consent_ids)
dim(tgct_res)
# 873

case_ids <- sort(unique(tgct_res$person_id))

write.table(case_ids, here("tgct", "data", "tgct_f4_case_ids.txt"), quote = FALSE, row.names = FALSE,
            col.names = FALSE)

table(tgct_res$batch)
# 1   2   3
# 163 644  66

f4_cases <- tgct_res %>% filter(batch == 3)
dim(f4_cases)
# 66

f4_case_ids <- sort(unique(f4_cases$person_id))

write.table(f4_case_ids, here("tgct", "data", "tgct_f4_only_case_ids.txt"), quote = FALSE, row.names = FALSE,
            col.names = FALSE)
...
tgct_controls <- select_controls_f4(
    control_name = "tgct4",
...
controls <- tgct_controls$final_controls
dim(controls)
# 9507

### should chekc not in PMCR
pmcr_ids <- unique(pmcr_all$person_id)
controls2 <- controls %>% filter(!(person_id %in% pmcr_ids))
dim(controls2)
# 9342
...
### exclude KT from the controls and add consent
kt_ids <- unique(kt_mrn$PMBB_ID)
controls3 <- controls2 %>% filter(!(person_id %in% kt_ids)) %>%
    filter(person_id %in% consent_ids)
dim(controls3)
# 8866

controls4 <- controls3 %>% filter(!(person_id %in% case_ids))
dim(controls4)
# 8866
# no overlap

write.table(controls4, here("tgct", "data", "tgct_f4_controls_df.txt"), quote = FALSE, row.names = FALSE,
            col.names = FALSE)

final_ids <- sort(unique(controls4$person_id))
length(final_ids)
# 8866

write.table(final_ids, here("tgct", "data", "tgct_f4_controls_ids.txt"), quote = FALSE, row.names = FALSE,
            col.names = FALSE)

# # ========================
# # COMPARE W/ ICD CODES - OLD
# # ========================
