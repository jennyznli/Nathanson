# ============================================================
# Cancer Case Selection Script
# ============================================================
# This script selects cancer cases from PMBB phenotype files
# using ICD codes, filtering by visit frequency, time span
# between first and last occurrence, gender, and age.
#
# Usage:
#   select_cancer_cases(
#       cancer_name = "breast",              # Name of the cancer type as string
#       icd_codes = c("^C50", "^Z85.3"),     # ICD9/10 code patterns (regex-compatible)
#       gender_filter = "All",               # "Male", "Female", or NULL
#       crep_filter = TRUE,                  # TRUE - include only CREP, FALSE - exclude CREP, NULL - no filter for CREP
...
#       exclude = FALSE,                     # Whether to exclude patients with certain ICD codes (those w/ history of other cancer)
...
#       output_prefix = NULL                 # Filename prefix (defaults to cancer_name)
#   )
#
# Output:
...
select_cancer_cases <- function(
    ...
    if (is.null(output_prefix)) {
        output_prefix <- tolower(gsub("[^A-Za-z0-9]", "_", cancer_name))
    }
    ...
    cat("Select cases for", cancer_name, "cancer log\n")
    cat("========================================", "\n")
    ...
    if (exclude) {
        cat("Other Cancer Codes to Exclude:", paste(exclude_codes, collapse = ", "), "\n")
    }
    ...
    # Function to exclude patients with other cancer types
    exclude_patients <- function(patient_data, condition_data, target_codes, exclude_codes, min_exclude_visits, min_exclude_timespan) {
        ...
        # Create pattern for other cancers (excluding our target codes)
        exclude_icd_pattern <- paste(exclude_codes, collapse = "|")
        target_pattern <- paste(target_codes, collapse = "|")

        # Find all occurrences of other cancer codes
        ...
        # Remove occurrences that match our target cancer (to avoid excluding our target cancer)
        exclude_occurrences_filtered <- exclude_occurrences[
            !grepl(target_pattern, condition_source_value, perl = TRUE, ignore.case = TRUE)
        ]
        cat("- Other exclusion occurrences excluding target cancer:", nrow(exclude_occurrences_filtered), "\n\n")
        ...
        exclude_summary <- create_patient_summary(exclude_occurrences_filtered, "other cancer")
        ...
        # Get patients to exclude (those with substantial other cancer evidence)
        ...
        cat("Target cancer patients who also have substantial other cancers:", length(overlap_patients), "\n")

        # Exclude these patients
        clean_patients <- setdiff(target_patients, patients_to_exclude)
        cat("Patients remaining after substantial other cancer exclusion:", length(clean_patients), "\n")
    ...
    all_patients <- analyze_condition_patients(occ, combined_pattern, paste(cancer_name, "cancer"))
    results <- list()
